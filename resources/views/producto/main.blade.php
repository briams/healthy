    ...
    <div class="navslide navwrap" id="app_content_toolbar">
        <div class="ui menu icon borderless grid" data-color="inverted white">
            <div class="item ui colhidden">
                <button id="new_producto" class="ui button compact"><i class="icon plus"></i>Nueva Producto
                </button>
            </div>
    ...
    <script type="text/javascript">
        ...
        var mainDataSource = new kendo.data.DataSource({
            transport: {
                read: function (options) {
                    ...
                    // };
                    dataSourceBinding(options, "{{ url('producto/get-main-list') }}")
                }
            },
        ...
        var grid = $("#grid").kendoGrid({
            ...
            columns: [
                ...
                {field: "pro_codigo", title: 'CODIGO', width: '80px'},
                {field: "pro_nombre", title: 'NOMBRE', width: '80px'},
                {field: "umd_descripcion", title: 'UNIDAD MEDIDA', width: '80px'},
                {field: "pro_laboratorio", title: 'LABORATORIO', width: '80px'},
            ...
            dataBound: function (sender, args) {
                ...
                $('.ajxEdit').click(function(e){
                    ...
                    window.location.href="{{ url('producto/editar') }}/"+id;
                });
                ...
                $('.ajxDown').click(function(e){
                    ...
                    var id = $(this).attr('data-idProd');
                    $.ajax({
                        url : "{{ action('ProductoController@bloquear') }}",
                        data : { id : id },
                        type : 'POST',
                        success : function(response){
                            if (response.status == STATUS_FAIL) {
                                ...
                                toast('success',3000,'Producto Bloqueado');
                                mainDataSource.read();
                            }
                        },
                        statusCode : {
                ...
                $('.ajxUp').click(function(e){
                    ...
                    var id = $(this).attr('data-idProd');
                    $.ajax({
                        url : "{{ action('ProductoController@activar') }}",
                        data : { id : id },
                        type : 'POST',
                        success : function(response){
                            if (response.status == STATUS_FAIL) {
                                ...
                                toast('success',3000,'Producto Activado');
                                mainDataSource.read();
                            }
                        },
                        statusCode : {
                ...
                $('.ajxDelete').click(function(e){
                    ...
                    var id = $(this).attr('data-idProd');
                    $.ajax({
                        url : "{{ action('ProductoController@eliminar') }}",
                        data : { id : id },
                        type : 'POST',
                        success : function(response){
                            if (response.status == STATUS_FAIL) {
                                ...
                                toast('success',3000,'Producto Eliminado');
                                mainDataSource.read();
                            }
                        },
                        statusCode : {

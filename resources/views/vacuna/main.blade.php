    ...
    <div class="navslide navwrap" id="app_content_toolbar">
        <div class="ui menu icon borderless grid" data-color="inverted white">
            <div class="item ui colhidden">
                <button id="new_vacuna" class="ui button compact"><i class="icon plus"></i>Nueva Vacuna
                </button>
            </div>
    ...
    <script type="text/javascript">
        ...
        var mainDataSource = new kendo.data.DataSource({
            transport: {
                read: function (options) {
                    ...
                    // };
                    dataSourceBinding(options, "{{ url('vacuna/get-main-list') }}")
                }
            },
        ...
        var grid = $("#grid").kendoGrid({
            ...
            columns: [
                ...
                {field: "vac_descripcion", title: 'DESCRIPCION', width: '80px'},
                {field: "vac_abreviatura", title: 'ABREVIATURA', width: '80px'},
            ...
            dataBound: function (sender, args) {
                ...
                $('.ajxEdit').click(function(e){
                    e.preventDefault();
                    var id = $(this).attr('data-idVac');
                    window.location.href="{{ url('vacuna/editar') }}/"+id;
                });
                ...
                $('.ajxDown').click(function(e){
                    ...
                    var id = $(this).attr('data-idVac');
                    $.ajax({
                        url : "{{ action('VacunaController@bloquear') }}",
                        data : { id : id },
                        type : 'POST',
                        success : function(response){
                            if (response.status == STATUS_FAIL) {
                                ...
                                toast('success',3000,'Vacuna Bloqueada');
                                mainDataSource.read();
                            }
                        },
                        statusCode : {
                ...
                $('.ajxUp').click(function(e){
                    ...
                    var id = $(this).attr('data-idVac');
                    $.ajax({
                        url : "{{ action('VacunaController@activar') }}",
                        data : { id : id },
                        type : 'POST',
                        success : function(response){
                            if (response.status == STATUS_FAIL) {
                                ...
                                toast('success',3000,'Vacuna Activada');
                                mainDataSource.read();
                            }
                        },
                        statusCode : {
                ...
                $('.ajxDelete').click(function(e){
                    ...
                    var id = $(this).attr('data-idVac');
                    $.ajax({
                        url : "{{ action('VacunaController@eliminar') }}",
                        data : { id : id },
                        type : 'POST',
                        success : function(response){
                            if (response.status == STATUS_FAIL) {
                                ...
                                toast('success',3000,'Vacuna Eliminada');
                                mainDataSource.read();
                            }
                        },
                        statusCode : {
        ...
        $(document).ready(function () {
            ...
            $('#new_vacuna').click(function (e) {
                window.location.href = "{{ url('vacuna/editar') }}";
            });
